/**
 * Preview shares Production and skips migrate, so owner-intelligence
 * reads/writes first ensure the additive attribution columns and
 * Business Health / Marketing / Reviews tables exist.
 */
import type { Prisma, PrismaClient } from "@prisma/client";

type IntelligenceClient = PrismaClient | Prisma.TransactionClient;

export const OWNER_INTELLIGENCE_ENSURE_SQL = [
...
export const OWNER_INTELLIGENCE_CONSTRAINTS_SQL = `
DO $$
BEGIN
...
export function resetOwnerIntelligenceSchemaEnsure() {
  ensurePromise = null;
}

export async function ensureOwnerIntelligenceSchema(db: IntelligenceClient) {
  if (!ensurePromise) {
    ensurePromise = (async () => {
      for (const statement of OWNER_INTELLIGENCE_ENSURE_SQL) {
        await db.$executeRawUnsafe(statement);
      }
      await db.$executeRawUnsafe(OWNER_INTELLIGENCE_CONSTRAINTS_SQL);
    })().catch((error) => {
      ensurePromise = null;
      throw error;
    });
