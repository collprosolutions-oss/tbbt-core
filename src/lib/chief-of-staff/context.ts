 ...
 * BUSINESS_PROTECTION load a bounded projection only when selected.
...
export function loadBusinessProtectionDeep(): never {
  ...
  throw new Error(
    "Business Protection specialist loads a bounded projection when selected and must not use the workspace deep-loader.",
  );
}
