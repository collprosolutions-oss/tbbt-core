...
try {
  ...
  check("Planner selects KNOWLEDGE_LAUNCH from existing recs", planSpecialists({ question: "How is my business doing?", activeRecommendationKeys: ["approve-business-knowledge"] }).selectedIds.includes("KNOWLEDGE_LAUNCH"));
  ...
  check("UNREVIEWED stays UNREVIEWED", byId[seededA.unreviewed.id]?.approvalState === "UNREVIEWED");
  check("APPROVED stays APPROVED", byId[seededA.approved.id]?.approvalState === "APPROVED");
  check("UNREVIEWED never becomes APPROVED", stateProjection.entries.every((row) => row.id !== seededA.unreviewed.id || row.approvalState !== "APPROVED"));
  check("CONFLICT remains CONFLICT", byId[seededA.conflict.id]?.trustState === "CONFLICT");
  check("ESTIMATE stays ESTIMATE", byId[seededA.estimate.id]?.trustState === "ESTIMATE");
  check("UNKNOWN stays UNKNOWN", byId[seededA.unreviewed.id]?.trustState === "UNKNOWN");
  check("SUPPORTED stays SUPPORTED", byId[seededA.supported.id]?.trustState === "SUPPORTED");
  check("EXTERNAL_REFERENCE stays EXTERNAL_REFERENCE", byId[seededA.external.id]?.sourceType === "EXTERNAL_REFERENCE");
  check("SYSTEM_DERIVED stays reserved", byId[seededA.systemDerived.id]?.sourceType === "SYSTEM_DERIVED");
