 ...
 *     every one of the 9 supported pages.
...
const PAGES = ["/dashboard", "/requests", "/customers", "/estimates", "/jobs", "/invoices", "/services", "/time-cards", "/payroll"];
...
async function main() {
  ...
  console.log("\nTEST 1e -- V2 region / compression / icon tokens are page-aware and bounded");
  {
    ...
    check("Time Cards regions are the real Time Cards boxes (Top Summary / Navigation / Table / Worker Detail / Page Spacing)",
      FOUNDER_REGIONS["time-cards"].map((r) => r.label).join("|") ===
        "Top Summary|Time Card Navigation / Filters|Time Entry Table|Worker Detail|Page Spacing");
  ...
  console.log("\nTEST 2 -- The founder sees the trigger on all 9 supported pages");
  ...
  console.log("\nTEST 3 -- Subscriber OWNER never sees the trigger, on any of the 9 pages (direct URL)");
  for (const path of PAGES) {
